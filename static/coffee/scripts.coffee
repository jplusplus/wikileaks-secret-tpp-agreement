class Map
	...
	init: =>
		@map = L.mapbox.map("map", "vied12.g9ekj0p2").setView([40, -74.50], 2)
	...
	loadedDataCallback : (a, matrice) =>
		...
		for country, relations of matrice
			for other_country, data of relations
				...
				m = new L.Point(a.x+(b.x-a.x)/2, a.y+(b.y-a.y)/2)
				m.y = m.y + 50
				@lines.push
					points: [a,m,b]
					data   : data
	...
	update : =>
		...
		@curves.enter()
			...
			.attr("class", "curve")
			.attr('stroke', '#00DBDB')
			# .attr('stroke-width', (d) -> return d.occ * .03)
			.attr('stroke-width', 1)
			# .attr('opacity',      (d) -> return d.occ/@MAX_OCC * .6)
			.attr('opacity',      1)
			.attr('fill', 'none')
