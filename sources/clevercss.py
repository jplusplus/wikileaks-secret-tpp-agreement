...
class Parser(object):
    ...
    def expand_property(self,  name, value):
    ...
    def preparse(self, source):
        ...
        def parse_definition():
            ...
            res = []
            for name, value in self.expand_property(m.group(1), m.group(2)):
                res.append([lineiter.lineno, name, value])
            return res
        ...
        for lineno, line in lineiter:
            ...
            # root and rules
                ...
                # otherwise parse a style definition.
                    ...
                    for definition in parse_definition():
                        rule[2].append(definition)

            # group blocks
            elif state_stack[-1] == 'group_block':
                for definition in parse_definition():
                    group_block_stack[-1][1].append(definition)
